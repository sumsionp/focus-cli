...
class FocusCLI:
    ...
    def get_daily_summary(self):
        ...
        for line in lines:
            ...
            marker_match = self.is_task(clean)
            if marker_match:
        ...
        for line in lines:
            ...
            if not line.startswith('  '):
                ...
                if last_entry_content and last_entry_content in active_entries:
                    ...
                    sub_marker_match = self.is_task(note)
                    if sub_marker_match:
    ...
    def _get_progress_stats(self, focus_item, parent_item):
        ...
        if parent_item is None:
            ...
            for line in parent_item['notes']:
                ...
                marker_match = self.is_task(line.strip())
                if marker_match:
                    total += 1
                    state = marker_match.group(1).strip()
                    if state in ['x', '-', '>']:
    ...
    def render_focus(self):
        ...
        if root_id != self.last_recorded_focus:
            if not focus_path:
                ...
                item_to_record['notes'] = [n for n in item_to_record['notes'] if not self.is_task(n)]
